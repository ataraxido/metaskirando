...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:output method="html" indent="yes"/>
	<xsl:param name="urlCss">bra2023.css</xsl:param>
	<xsl:param name="urlBlob">https://rpcache-aa.meteofrance.com/gdss/v1/metronome_bra/blob?token=eyJjbGFzcyI6ImludGVybmV0IiwiYWxnIjoiSFMyNTYiLCJ0eXAiOiJKV1QifQ.eyJqdGkiOiIyMmExNjNmOWRlMmFiNzQ1ZGExNDE3NDE4ODRlMjdjOCIsImlhdCI6MTcwMjU1NDY5NH0.5ZKBHG2QEh_tnPqmiZDtewy0zs-QwwG6SrskLDY1M28&amp;blob_filename=</xsl:param>
	<xsl:param name="CheminPicto">https://meteofrance.com/modules/custom/mf_map_layers_v2/assets/images/</xsl:param>
  <!-- Version -->
  <!-- Version 3.0.7 -->
  <!-- VersionExe 3.0 -->
      ...
      <div class="row">
        ...
        <div class="col1">
              ...
              <xsl:for-each select="NEIGEFRAICHE/NEIGE24H">
              	le <xsl:value-of select="substring-before(@DATE,'T')"></xsl:value-of>
              	entre <xsl:value-of select="@SS24Min"></xsl:value-of>cm
              	et <xsl:value-of select="@SS24Max"></xsl:value-of>cm<br/>        	
              </xsl:for-each>
        </div>
        <!-- Rubrique météo -->
        <div class="col2">
  ...
  <xsl:template match="CARTOUCHERISQUE">
	...
	<!--	</div>
      ...
      <div class="figurineRisque">
      	<p>
      		<xsl:value-of select="RISQUE/@LOC1"/> Risque <xsl:value-of select="RISQUE/@RISQUE1"/>
      		<xsl:if test="EVOLURISQUE1!=''"> evoluant <xsl:value-of select="RISQUE/@EVOLURISQUE1"/></xsl:if><br/>
      	   <xsl:value-of select="RISQUE/@LOC2"/> Risque <xsl:value-of select="RISQUE/@RISQUE2"/>
      		<xsl:if test="EVOLURISQUE2!=''"> evoluant <xsl:value-of select="RISQUE/@EVOLURISQUE2"/></xsl:if>
      	</p>
      </div>
      <!--Figurine-->
      <div class="figurinePente">
        <p>
				...
				<xsl:if test="PENTE/@N='true'"><br/><xsl:text>N </xsl:text></xsl:if>
				<xsl:if test="PENTE/@NE='true'"><br/><xsl:text>NE </xsl:text></xsl:if>
				<xsl:if test="PENTE/@E='true'"><br/><xsl:text>E </xsl:text></xsl:if>
				<xsl:if test="PENTE/@SE='true'"><br/><xsl:text>SE </xsl:text></xsl:if>
				<xsl:if test="PENTE/@S='true'"><br/><xsl:text>S </xsl:text></xsl:if>
				<xsl:if test="PENTE/@SO='true'"><br/><xsl:text>SO </xsl:text></xsl:if>
				<xsl:if test="PENTE/@O='true'"><br/><xsl:text>O </xsl:text></xsl:if>
				<xsl:if test="PENTE/@NO='true'"><br/><xsl:text>NO </xsl:text></xsl:if>
			</p>
      </div>
		</div>
      <!--catouche-->
