...
/*
    ...
    This prini_set('display_errors', '1');
ini_set('display_startup_errors', '1');
error_reporting(E_ALL);ogram is distributed in the hope that it will be useful,
...
function make_region_list(&$sorties)
{
	...
	for ($i=0;$i<$n;$i++) {
		...
		$j = 0; $found = FALSE;
		while($j<$r) {
			if (mb_eregi($reg[$j]['key'],$reg_name)) {
				$reg[$j]['nbr'] ++;
				$found = TRUE;
			}
			$j++;
...
echo "load " . strlen($txttmp) . " data<br/>";
  		...
  		} elseif (time() > (@filemtime($ftmp) + 360)) // efface le fichier tmp si ca fait trop longtemps...
			@unlink($ftmp);
	}
	return TRUE;
}
...
function reset_Sto($nread = 120, $base = 'sto' ) {
  global $SETTINGS;
...
function download_Sto($base = 'sto') {
   global $SETTINGS;
	$web  = $SETTINGS['odir'] . "/$base.web";
	$textall = file_get_contents("https://skitourenportal.eu/");
	$fd=@fopen($web,'w');
...
function parse_Sto(&$textall, $last_id) {
	$p1 = strpos($textall,'<table class="w-full border-collapse border-none text-sm"');
	$p1 = strpos($textall,'<tbody', $p1);
	$p2 = strpos($textall,'</div>',$p1);
   ...
   $new_id = $last_id;
	...
	for ($i = 1;$i<$n; $i++) {
		...
		$p1 = strpos($entries[$i],'href="')+6;
		$p2 = strpos($entries[$i],'"',$p1);
		$link = substr($entries[$i],$p1,$p2-$p1);
		$p1 = strpos($link,'beitrag/')+8;
		$id = substr($link,$p1);
		
		$date = trim(strip_tags('<td'.$items[1]));
		$dt = substr($date,6,4) . "-" . substr($date,3,2) . "-" . substr($date,0,2);
		$reg = trim(strip_tags('<td'.$items[2]));
		$name = trim(strip_tags('<td'.$items[3]));
		$outing = trim(strip_tags('<td'.$items[4]));
...
// on garde que la partie intéressante.
	...
	for($i=1;$i<$n;$i++)	{
		...
		mb_ereg("sortie([0-9]+)[.]html",$items[1],$regs);
		$id = $regs[1];
//		if ($id > $last_id) {
...
function parse_Skitour(&$textall,$last_id)
{
	global $dlim;
   global $SETTINGS;
   $obj = json_decode(removeBOM($textall), true);
...
//	mb_ereg('([0-9]+)',$entries[1],$regs);	$new_id = $regs[1];

//	$fxtx  = $SETTINGS['odir'] . "/skitour.txt";
//	$textcur = file_get_contents($fxtx);
	$n=count($obj)-1;
	for($i=1;$i<$n;$i++)	{
		$items = $obj[$i];
  		$id=$items["id"];
//		if (strpos($textcur,"skitour ".$id) == false) {
			$nom = $items["auteur"]["pseudo"];
			$voie = $items["titre"];
			$reg = $items["massif"]["nom"];
			$cot = $items["dif_ski"];
			$date = date("Y-m-d", $items["date"]);
			if ($date < $dlim) break;	// pas plus vieux que 1 mois.
			$textall .= "skitour $id\n$date $cot\n$voie\nhttps://skitour.fr/sorties/$id\n$reg\n$nom\n";
			if ($id > $last_id) $last_id = $id;
//		}
	}
	return $last_id;
...
function parse_Gipfelbuch(&$textall,$last_id)
{
	global $dlim;
// on garde que la partie int?essante.
	$entries = explode('<article class="item list-item',substr($textall,strpos($textall,'data-target="conditionsview" class="listcontainer newstyletable>"')));
	$textall = '';
//tableaux de traduction
	...
	for($i=1;$i<$n;$i++)
	{
		$items = explode('dv-tt',str_replace(">","]",str_replace("<","[",$entries[$i])));
		
		$id = substr($items[0],strpos($items[0],"id=")+4);
		$id = substr($id,0,strpos($id,'"'));
		$id = str_replace("conditions","",$id);
		$lien = substr($items[0],strpos($items[0],"href=")+6);
		// keep only skitouring base on url
		if(strpos($lien, "-skitour-")>0) {
			$lien = "https://www.gipfelbuch.ch".substr($lien,0,strpos($lien,'"'));
			$nom = substr($items[4],strpos($items[4],"]")+1);
			$nom = substr($nom,0,strpos($nom,"["));
			$voie = substr($items[10],strpos($items[10],"]")+1);
			$voie = substr($voie,0,strpos($voie,"["));
			$reg = substr($items[11],strpos($items[11],"]")+1);
			$reg = substr($reg,0,strpos($reg,"["));
			$reg2 = str_replace($trad_ch, $trad_fr, $reg);
			$date = substr($items[18],strpos($items[18],"]")+1);
			$date = substr($date,strpos($date,"]")+1);
			$date = substr($date,0,strpos($date,"["));
			$date = trim($date);
			mb_ereg ("([0-9]{2}).([0-9]{2}).([0-9]{4})", $date, $regs);
