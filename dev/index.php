...
*/
  require_once('./settings.inc.php');
	...
	$time = time();
	unset( $last_sktr, $last_volo, $last_bivk, $last_skrd, $last_gipf, $last_sto );
// les dernieres sorties sont stockees dans 'last' pour 2 jours.
	...
	$last_sto = trim(@file_get_contents($SETTINGS['odir'] . '/skitouren.last'));
// on efface le cookie 'last' pour le mettre a jour.
	setcookie('last','',$time-1000);
	setcookie('last',"sktr=$last_sktr&volo=$last_volo&bivk=$last_bivk&skrd=$last_skrd&gipf=$last_gipf&sto=$last_sto",$time+48*3600);
// les dernieres sorties vues sont stockees dans 'current' pour 5 min.
	if (isset($_COOKIE['current']))
	{
		...
		$last_sto = substr($items[5],strpos($items[5],'=')+1);
	}
...
if (!empty($_GET['str']))
{
	...
	$nf = 0;
	$n = count($sorties);
	$req_str = sans_accent(mb_eregi_replace('([1-5])[.]([1-6])',"\\1[.]\\2",$_GET['str']));
	...
	for($i = 0; $i < $n; $i++)
	{
		if (kick_zeurch($keys,sans_accent(implode(' ',$sorties[$i]))))
		{
			...
			$found[$nf]['date'] = mb_eregi_replace($req_str,"<span class='yt'>\\0</span>",$sorties[$i]['date']);
			$found[$nf]['nom'] = mb_eregi_replace($req_str,"<span class='yt'>\\0</span>",$sorties[$i]['nom']);
			$found[$nf]['reg'] = mb_eregi_replace($req_str,"<span class='yt'>\\0</span>",$sorties[$i]['reg']);
			$found[$nf]['part'] = mb_eregi_replace($req_str,"<span class='yt'>\\0</span>",$sorties[$i]['part']);
			$found[$nf]['cot'] = mb_eregi_replace($req_str,"<span class='yt'>\\0</span>",$sorties[$i]['cot']);
...
if (isset($found))
{
	...
	for ($i=0;$i<$n;$i++)
	{
		...
		echo "$trtag<td><a target=\"_$site\" href=\"$lien\">$nom</a></td><td><b>$reg</b></td><td>$cot</td><td><i> $part</i> [$site]</td></tr>\n";
	}
